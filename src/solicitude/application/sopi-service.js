...
const {
  findAllPermisionFromProfileId,
  findAllPermissionsFromUserAndProfile,
} = require("../../auth/domain/permission-repository");
const { sequelize } = require("../../database/db-init");
const {
  getAllPurchasesWithManager,
} = require("../../purchases/domain/purchase-repository");
...
const {
  ...
  findSopi,
  getAllSopis,
  getAllSopisByConditions,
  getSopiById,
  getAllSopisByStatus,
  getSopyByConditions,
} = require("../domain/sopi-repository");
const {
  saveSopiDetail,
  getSopiDetailsById,
} = require("../domain/sopidetail-repository");
const {
  addLogEntryByStatusName,
  addLogEntryByStatusId,
} = require("../domain/sopilog-repository");
...
const generateSopiDetails = async (items, sopiId) => {
  ...
  for (let item of items) {
    if (item.supplyId) {
      ...
      const sopiDetailCreated = await saveSopiDetail({
...
const createSopiSeqTransactional = async ({
  ...
  try {
    ...
    const response = await sequelize.transaction(async (t) => {
      ...
      let sopiCreated = await saveSopi({
        costCenterId,
        financingId,
        basis: "",
        userId,
      ...
      sopiCreated = await updateSopi(sopiCreated.id, {
        basis: basis || null,
      }).catch((e) => {
        throw new Error("Fundamento debe existir");
      });
...
const updateSopiWithStatus = async ({
  sopiId,
  statusId,
...
const getSopiByIdWithDetails = async (sopiId, profileId, userId) => {
  try {
    const access = await findAllPermissionsFromUserAndProfile(
      userId,
      profileId
    );
    ...
    if (access.find((a) => a.name == "SOPI_VER")) {
      ...
      access.find((a) => a.name == "SOPI_EDITAR_ESTADO_REVISADO_REFERENTE")
    ) {
      let where = {
        id: { [Op.eq]: `${sopiId}` },
        [Op.and]: [{ sopiStatusId: 4 }],
      };
      const sopi = await getSopyByConditions(where);
      if (sopi === null) {
...
const getSopisFilteredByUserPermissions = async (profileId, userId) => {
  ...
  const sopiStatusAccess = access.filter((access) =>
    access.name.includes("SOPI_VER_ESTADO")
  );
...
const getSopisFilteredByUserPermissions2 = async (query, profileId, userId) => {
  ...
  if (access.find((a) => a.name == "SOPI_EDITAR_ESTADO_REVISADO_REFERENTE")) {
    sopis = await getAllSopisByStatus(
      { name: "REVISION_REFERENTE" },
      page,
      perPage
    );
